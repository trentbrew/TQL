 ...
 * Validate step-specific requirements
 */
function validateStepRequirements(step: StepSpec, availableDatasets: Set<string>): void {
  switch (step.type) {
    case 'source':
 ...
 */
export function getStepDependencies(step: StepSpec, plan: ExecutionPlan): StepSpec[] {
  const { steps, order } = plan;
  const stepMap = new Map(steps.map(s => [s.id, s]));

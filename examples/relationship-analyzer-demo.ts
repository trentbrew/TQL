...
async function runDemo() {
  ...
  const photosData = JSON.parse(
    readFileSync('./data/real-photos.json', 'utf-8'),
  );
  const albumsData = JSON.parse(
    readFileSync('./data/real-albums.json', 'utf-8'),
  );
  ...
  relationships.forEach((rel, index) => {
    ...
    console.log(
      `  Source: ${rel.sourceDataset}.${rel.sourceType}.${rel.sourceAttribute}`,
    );
    console.log(
      `  Target: ${rel.targetDataset}.${rel.targetType}.${rel.targetAttribute}`,
    );
    console.log(`  Type: ${rel.relationship.type}`);
    console.log(
      `  Confidence: ${(rel.relationship.confidence * 100).toFixed(1)}%`,
    );
    ...
    if (rel.examples.length > 0) {
      ...
      for (let i = 0; i < exampleCount; i++) {
        ...
        if (example) {
          const sourceStr =
            typeof example.source === 'object'
              ? JSON.stringify(example.source)
              : example.source;
          const targetStr =
            typeof example.target === 'object'
              ? JSON.stringify(example.target)
              : example.target;
  ...
  const determineAttributeType = (attrName: string): string => {
    ...
    if (attrName.toLowerCase().includes('id')) attrType = 'number';
      ...
      attrName.toLowerCase().includes('count') ||
  ...
  console.log(
    `Processing ${relationships.length} relationships for diagram...`,
  );
  ...
  relationships.forEach((rel) => {
    ...
    if (rel.relationship.confidence < 0.6) {
      console.log(
        `Skipping low confidence relationship: ${rel.sourceAttribute} -> ${rel.targetAttribute} (${rel.relationship.confidence})`,
      );
      return;
    }
    ...
    if (sourceEntity && targetEntity) {
      ...
      sourceEntity.attributeTypes.set(
        rel.sourceAttribute,
        determineAttributeType(rel.sourceAttribute),
      );

      targetEntity.attributes.add(rel.targetAttribute);
      targetEntity.attributeTypes.set(
        rel.targetAttribute,
        determineAttributeType(rel.targetAttribute),
      );
      ...
      if (rel.relationship.type === 'foreignKey') {
        ...
        if (rel.sourceAttribute === 'userId' && rel.targetAttribute === 'id') {
          ...
          rel.sourceAttribute === 'albumId' &&
          rel.targetAttribute === 'id'
        ) {
        ...
        console.log(
          `Adding relationship: ${relationshipKey} (${relationshipType})`,
        );
      } else if (
        rel.relationship.type === 'sameAttribute' &&
        rel.relationship.confidence >= 0.8
      ) {
        ...
        let description = '';
        if (
          rel.sourceAttribute === 'userId' &&
          rel.targetAttribute === 'userId'
        ) {
        ...
        console.log(
          `Adding same attribute relationship: ${relationshipKey} (${relationshipType})`,
        );
      }
    }
  });
  ...
  entities.forEach((entity) => {
    ...
    const sortedAttributes = Array.from(entity.attributes).sort((a, b) => {
      ...
      if (a.toLowerCase().includes('id') && !b.toLowerCase().includes('id'))
        return -1;
      if (!a.toLowerCase().includes('id') && b.toLowerCase().includes('id'))
        return 1;

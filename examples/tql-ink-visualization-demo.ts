...
async function runTQLInkDemo() {
  ...
  // Sample query
  const sampleQuery =
    'FIND post AS ?p, user AS ?u WHERE ?p.userId = ?u.id RETURN ?u.name, ?p.title, ?p.views ORDER BY ?p.views DESC';
          ...
          output: {
            ...
            queryPlan: {
              steps: [
                ...
                {
                  operation: 'PROJECT',
                  attributes: ['?u.name', '?p.title', '?p.views'],
                },
  ...
  console.log(
    "(Press 'h' to see keyboard controls, numbers to expand nodes, 'q' to quit)",
  );

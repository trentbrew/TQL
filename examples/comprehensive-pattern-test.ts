...
async function runComprehensiveTests() {
  ...
  console.log(
    chalk.green(
      `✅ Store initialized with ${stats.totalFacts} facts across ${stats.uniqueEntities} entities\n`,
    ),
  );
  ...
  for (const test of comprehensiveTests) {
    ...
    try {
      ...
      if (result.count === test.expectedCount) {
        console.log(
          chalk.green(
            `✅ PASS: Expected ${test.expectedCount}, got ${result.count}`,
          ),
        );
        passCount++;
      } else {
        console.log(
          chalk.red(
            `❌ FAIL: Expected ${test.expectedCount}, got ${result.count}`,
          ),
        );
        if (result.rows.length > 0) {
          console.log(
            `Sample results: ${chalk.cyan(JSON.stringify(result.rows.slice(0, 2)))}`,
          );
        }
      }
    } catch (error) {
  ...
  const catalog = [
    {
      attribute: 'name',
      type: 'string',
      examples: ['iPhone 15', 'Alice Johnson'],
    },
    { attribute: 'email', type: 'string', examples: ['alice@example.com'] },
    { attribute: 'price', type: 'number', examples: [999.99, 4.5] },
    {
      attribute: 'category',
      type: 'string',
      examples: ['electronics', 'food'],
    },
    ...
    try {
      ...
      if (result.error) {
        ...
        if (
          ...
          !result.eqlsQuery
            .toLowerCase()
            .includes(test.shouldFind.toLowerCase())
        ) {
          valid = false;
        }
        ...
        if (valid && test.shouldWork) {
          ...
          try {
            ...
            console.log(
              `Execution result: ${chalk.green(`${execResult.count} items found`)}`,
            );
          } catch (execError) {
            console.log(
              chalk.yellow(
                `⚠️ Query generated but execution failed: ${execError instanceof Error ? execError.message : String(execError)}`,
              ),
            );
          }
        } else {
          console.log(
            chalk.red(
              `❌ FAIL: Generated query doesn't match expected pattern`,
            ),
          );
        }
      }
  ...
  console.log(
    `Pattern Tests: ${chalk.green(`${passCount}/${comprehensiveTests.length} passed`)} (${Math.round((passCount / comprehensiveTests.length) * 100)}%)`,
  );
  console.log(
    `Natural Language Tests: ${chalk.green(`${nlPassCount}/${naturalLanguageTests.length} passed`)} (${Math.round((nlPassCount / naturalLanguageTests.length) * 100)}%)`,
  );
  console.log(
    `Overall: ${chalk.green(`${passCount + nlPassCount}/${comprehensiveTests.length + naturalLanguageTests.length} passed`)} (${Math.round(((passCount + nlPassCount) / (comprehensiveTests.length + naturalLanguageTests.length)) * 100)}%)`,
  );

  if (
    passCount === comprehensiveTests.length &&
    nlPassCount === naturalLanguageTests.length
  ) {
    console.log(
    ...
    console.log(
      chalk.green(
        'TQL is robust and ready for production use with diverse data patterns!',
      ),
    );
  } else {
    console.log(

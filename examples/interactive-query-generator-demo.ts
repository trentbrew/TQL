...
const sampleFiles = [
  {
    name: 'posts.json',
    description: 'Blog posts with titles, content, and metadata',
  },
  {
    name: 'real-users.json',
    description: 'User profiles with contact information',
  },
  {
    name: 'products_webflow.json',
    description: 'Product catalog with prices and details',
  },
  {
    name: 'real-photos.json',
    description: 'Photo collection with URLs and album references',
  },
];
...
async function loadData(filePath: string): Promise<void> {
  console.log(
    `${colors.bright}📥 Loading data from:${colors.reset} ${filePath}`,
  );
  ...
  try {
    ...
    if (Array.isArray(jsonData)) {
      console.log(
        `${colors.dim}Array data detected with ${jsonData.length} items${colors.reset}`,
      );
...
async function runDemo(): Promise<void> {
  try {
    ...
    console.log(
      `${colors.bright}${colors.cyan}  TQL Query Generator Demo${colors.reset}`,
    );
    console.log(
      `${colors.bright}${colors.cyan}====================================${colors.reset}`,
    );
    ...
    console.log(
      'and suggests meaningful EQL-S queries based on the patterns it finds.',
    );
    ...
    for (let i = 0; i < sampleFiles.length; i++) {
      ...
      console.log(
        `  ${colors.yellow}${i + 1}.${colors.reset} ${file.name} - ${file.description}`,
      );
    }
    ...
    if (isNaN(index) || index < 0 || index >= sampleFiles.length) {
      console.log(
        `${colors.red}Invalid choice. Please enter a number between 1 and ${sampleFiles.length}.${colors.reset}`,
      );
    ...
    console.log('\n');
    console.log(
      `${colors.bright}${colors.blue}Generating query suggestions...${colors.reset}`,
    );
    ...
    if (suggestions.length === 0) {
      console.log(
        `${colors.red}No query suggestions could be generated for this data.${colors.reset}`,
      );
    ...
    if (
      isNaN(queryIndex) ||
      queryIndex < 0 ||
      queryIndex >= suggestions.length
    ) {
      console.log(
        `${colors.red}Invalid choice. Please enter a number between 1 and ${suggestions.length}.${colors.reset}`,
      );
      await prompt(`\nPress Enter to try again...`);
      return runDemo();
    }

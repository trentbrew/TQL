...
async function runQuery(store: EAVStore, query: string): Promise<any> {
  ...
  if (result.errors && result.errors.length > 0) {
    throw new Error(
      `Query parsing failed: ${result.errors[0]?.message || 'Unknown error'}`,
    );
  }
  ...
  const rows = queryResult.bindings.map((binding) => {
    ...
    return Array.from(result.projectionMap?.entries() || []).map(
      ([key, varName]) => {
...
async function runTests() {
  ...
  console.log(
    chalk.green(`✅ Store initialized with ${stats.totalFacts} facts\n`),
  );
  ...
  for (const testCase of regexTestCases) {
    ...
    try {
      ...
      if (
        allMatched &&
        matchCount === testCase.expectedMatches.length &&
        matchCount === familyValues.length
      ) {
        console.log(
          chalk.green(`✅ PASS: All expected matches found and nothing extra`),
        );
        regexPassCount++;
      } else if (allMatched && matchCount === testCase.expectedMatches.length) {
        console.log(
          chalk.yellow(
            `⚠️ PARTIAL PASS: All expected matches found, but extra results present`,
          ),
        );
        console.log(
          `Expected: ${chalk.cyan(JSON.stringify(testCase.expectedMatches))}`,
        );
      } else {
        ...
        console.log(
          `Missing: ${chalk.red(JSON.stringify(testCase.expectedMatches.filter((e) => !familyValues.includes(e))))}`,
        );
      }
    } catch (error) {
      console.log(
    ...
    try {
      ...
      if (result.error) {
        ...
        if (testCase.expectedPattern) {
          patternMatch =
            result.eqlsQuery.includes('MATCHES') &&
        ...
        if (patternMatch) {
          ...
          try {
            ...
            console.log(
              `Query execution results: ${chalk.green(queryResults.rows.length)} rows returned`,
            );
            if (queryResults.rows.length > 0) {
              console.log(
                `Sample result: ${chalk.cyan(JSON.stringify(queryResults.rows[0]))}`,
              );
            }
            nlPassCount++;
          } catch (queryError) {
            console.log(
              chalk.red(
                `❌ Generated query execution failed: ${queryError instanceof Error ? queryError.message : String(queryError)}`,
              ),
            );
          }
        } else {
          console.log(
            chalk.red(
              `❌ FAIL: Pattern not properly recognized in generated query`,
            ),
          );
          if (testCase.expectedPattern) {
            console.log(
              `Expected pattern: ${chalk.cyan(testCase.expectedPattern)}`,
            );
          }
        }
      }
  ...
  console.log(
    `Regex Pattern Tests: ${chalk.green(`${regexPassCount}/${regexTestCases.length} passed`)} (${Math.round((regexPassCount / regexTestCases.length) * 100)}%)`,
  );
  console.log(
    `Natural Language Tests: ${chalk.green(`${nlPassCount}/${nlTestCases.length} passed`)} (${Math.round((nlPassCount / nlTestCases.length) * 100)}%)`,
  );
  console.log(
    `Overall: ${chalk.green(`${regexPassCount + nlPassCount}/${regexTestCases.length + nlTestCases.length} passed`)} (${Math.round(((regexPassCount + nlPassCount) / (regexTestCases.length + nlTestCases.length)) * 100)}%)`,
  );

  if (
    regexPassCount === regexTestCases.length &&
    nlPassCount === nlTestCases.length
  ) {
    console.log(chalk.bgGreen.black('\n 🎉 ALL TESTS PASSED! \n'));
  } else {

...
async function diagnoseNLP() {
  ...
  const catalog = [
    {
      attribute: 'name',
      type: 'string',
      examples: ['Alice Johnson', 'iPhone 15'],
    },
  ...
  console.log(
    `📊 Test environment: ${dataStats.totalFacts} facts, ${dataStats.uniqueEntities} entities\n`,
  );

  // Helper function to test EQL-S syntax
  async function testEQLSyntax(
    eqlsQuery: string,
  ): Promise<{ valid: boolean; error?: string; executable?: boolean }> {
    try {
  ...
  for (const test of nlQueries) {
    ...
    try {
      ...
      if (nlResult.error) {
        ...
        if (
          nlResult.error.includes('Generated query does not start with FIND')
        ) {
      ...
      if (!syntaxResult.executable) {
        console.log(
          chalk.yellow(
            `⚠️ Query valid but not executable: ${syntaxResult.error}`,
          ),
        );
        ineffectiveQueries++;
        totalFailures++;
      ...
      const containsPattern = nlResult.eqlsQuery
        .toLowerCase()
        .includes(test.expectedPattern.toLowerCase());

      if (!containsPattern) {
        console.log(
          chalk.yellow(
            `⚠️ Query doesn't contain expected pattern "${test.expectedPattern}"`,
          ),
        );
        ineffectiveQueries++;
        totalFailures++;
  ...
  console.log(
    `Total Queries Tested: ${chalk.blue(nlQueries.length.toString())}`,
  );
  console.log(
    `Successful: ${chalk.green(successfulQueries.toString())} (${Math.round((successfulQueries / nlQueries.length) * 100)}%)`,
  );
  console.log(
    `Failed: ${chalk.red(totalFailures.toString())} (${Math.round((totalFailures / nlQueries.length) * 100)}%)`,
  );

  console.log(chalk.yellow('\nFailure Breakdown:'));
  console.log(
    `• Syntax Errors: ${chalk.red(syntaxErrors.toString())} (${Math.round((syntaxErrors / nlQueries.length) * 100)}%)`,
  );
  console.log(
    `• Ineffective Queries: ${chalk.yellow(ineffectiveQueries.toString())} (${Math.round((ineffectiveQueries / nlQueries.length) * 100)}%)`,
  );
  ...
  if (syntaxErrors > ineffectiveQueries) {
    ...
    console.log(
      '   → Focus on improving EQL-S generation to produce valid syntax',
    );
    ...
    console.log(
      chalk.yellow('⚖️ Mixed Issues: Both syntax and effectiveness problems'),
    );
    console.log(
      '   → Address both syntax generation and semantic understanding',
    );
  }
  ...
  if (ineffectiveQueries > 0) {
    ...
    console.log(
      '   → Consider fine-tuning prompts for specific query patterns',
    );
  }
}
